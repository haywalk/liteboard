...
class DB:
    ...
    def get_board(self, board_id: str) -> BoardSummary:
        ...
        result = cursor.execute(f'''
            SELECT thread, date, subject, attachment 
            FROM posts 
        ...
        for thread in threads:
            ...
            attachment = thread[3]
            board.add_thread(ThreadSummary(thread_id, date, subject, reply_count, attachment))
    ...
    def get_thread(self, board_id: str, thread_id: int) -> Thread:
        ...
        cursor = self.connection.cursor()
        result = cursor.execute(f'''
            SELECT date, subject, attachment
            FROM posts
            WHERE board = "{board_id}"
        ...
        ''')
        thread_date, thread_subject, attachment = result.fetchone()
        reply_count = self._count_replies(board_id, thread_id)
        thread_summary = ThreadSummary(thread_id, thread_date, thread_subject, reply_count, attachment)
        this_thread = Thread(thread_summary)

        # get posts
        result = cursor.execute(f'''
            SELECT date, postorder, contents, attachment
            FROM posts
            WHERE board = "{board_id}"
            AND thread = {thread_id}    
        ''')
        ...
        for row in post_rows:
            post_date, post_order, post_content, attachment = row
            this_post = Post(post_content, post_date, post_order, attachment)
    ...
    def new_thread(self, board_id: str, subject: str, content: str, attachment: str) -> int:
        ''' Create a new thread.
        ...
        result = cursor.execute(f'''
            INSERT INTO posts (
                board, thread, postorder, date, subject, contents, attachment
            )
            VALUES (
                "{board_id}",
                {thread_id},
    ...
    def reply_thread(self, board_id: str, thread_id: int, contents: str, attachment: str) -> bool:
        ''' Reply to a thread.
